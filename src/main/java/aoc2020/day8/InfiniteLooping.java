...
public class InfiniteLooping {
    ...
    private static void swap(Command c) {
        if (c.opcode == Instruction.JMP) c.opcode = Instruction.NOP;
        else if (c.opcode == Instruction.NOP) c.opcode = Instruction.JMP;
    }
    ...
    private static Integer part2(List<Command> data) {
        for (Command c : data) {
            if (c.opcode == Instruction.ACC) continue;
            swap(c);
    ...
    public static void main(String[] args) {
        List<Command> data = Input.readData(Year.AOC_2020, "day8.txt")
                .map(s -> s.split(" "))
